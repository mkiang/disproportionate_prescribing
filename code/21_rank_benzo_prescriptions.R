## 21_rank_benzo_prescriptions.R ----
##
## In each year, among *all* providers and the top 1% of providers, which
## benzos are the most common? We collapse opioids based on their
...
mme_convers <- load_mme_conversion() %>%
    dplyr::mutate(generic_name = snakecase::to_title_case(generic_name, parsing_option = 0))
...
## of prescriptions.
if (!fs::file_exists(here::here("data", "benzos_prescription_ranks.csv"))) {
    ...
    ranking_results <-
        foreach::foreach(
            ...
            temp_df <- import_mini_rx_data(year_x, "benzos")
    ...
    readr::write_csv(ranking_results,
                     here::here("data", "benzos_prescription_ranks.csv"))
}
